...
export function createDailyStepsStore(redis, key = DEFAULT_STEPS_KEY) {
  return {
    ...
    async saveMany(values) {
      if (!Array.isArray(values) || values.length < 1 || values.length > 45) {
        throw new Error('Step sync must contain between 1 and 45 daily totals.');
      }
      const entries = values.map((value) => normalizeDailySteps(value));
      const updates = {};
      for (const entry of entries) updates[`${entry.profile}:${entry.date}`] = entry;
